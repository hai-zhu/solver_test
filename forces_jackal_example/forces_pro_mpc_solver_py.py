#forces_pro_mpc_solver : A fast customized optimization solver.
#
...
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
   ...
   OUTPUT = forces_pro_mpc_solver_py.forces_pro_mpc_solver_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = forces_pro_mpc_solver_py.forces_pro_mpc_solver_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = forces_pro_mpc_solver_py.forces_pro_mpc_solver_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'forces_pro_mpc_solver/lib/forces_pro_mpc_solver.so')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'forces_pro_mpc_solver/lib/forces_pro_mpc_solver_withModel.so'))
    csolver = getattr(_lib,'forces_pro_mpc_solver_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'forces_pro_mpc_solver/lib/libforces_pro_mpc_solver_withModel.so'))
    csolver = getattr(_lib,'forces_pro_mpc_solver_solve')

class forces_pro_mpc_solver_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('xinit', ctypes.c_double * 5),
('x0', ctypes.c_double * 140),
('all_parameters', ctypes.c_double * 280),
]

forces_pro_mpc_solver_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
forces_pro_mpc_solver_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
}

class forces_pro_mpc_solver_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
forces_pro_mpc_solver_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class forces_pro_mpc_solver_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(forces_pro_mpc_solver_params_ctypes), ctypes.POINTER(forces_pro_mpc_solver_outputs_ctypes), ctypes.POINTER(forces_pro_mpc_solver_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def forces_pro_mpc_solver_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = forces_pro_mpc_solver_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=forces_pro_mpc_solver_params_types[par], requirements='F')
    ...
    outputs_py = forces_pro_mpc_solver_outputs_ctypes()
    info_py = forces_pro_mpc_solver_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.forces_pro_mpc_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.forces_pro_mpc_solver_casadi2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.forces_pro_mpc_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.forces_pro_mpc_solver_casadi2forces)
            else:
                exitflag = _lib.forces_pro_mpc_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.forces_pro_mpc_solver_casadi2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in forces_pro_mpc_solver_outputs:
        forces_pro_mpc_solver_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return forces_pro_mpc_solver_outputs,int(exitflag),info_py

solve = forces_pro_mpc_solver_solve
