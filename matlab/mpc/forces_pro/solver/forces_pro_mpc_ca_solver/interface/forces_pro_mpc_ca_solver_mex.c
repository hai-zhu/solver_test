...
#include "mex.h"
#include "math.h"
#include "../include/forces_pro_mpc_ca_solver.h"
...
extern void forces_pro_mpc_ca_solver_casadi2forces(forces_pro_mpc_ca_solver_float *x, forces_pro_mpc_ca_solver_float *y, forces_pro_mpc_ca_solver_float *l, forces_pro_mpc_ca_solver_float *p, forces_pro_mpc_ca_solver_float *f, forces_pro_mpc_ca_solver_float *nabla_f, forces_pro_mpc_ca_solver_float *c, forces_pro_mpc_ca_solver_float *nabla_c, forces_pro_mpc_ca_solver_float *h, forces_pro_mpc_ca_solver_float *nabla_h, forces_pro_mpc_ca_solver_float *hess, solver_int32_default stage, solver_int32_default iteration, solver_int32_default threadID);
forces_pro_mpc_ca_solver_extfunc pt2function_forces_pro_mpc_ca_solver = &forces_pro_mpc_ca_solver_casadi2forces;


/* Some memory for mex-function */
static forces_pro_mpc_ca_solver_params params;
static forces_pro_mpc_ca_solver_output output;
static forces_pro_mpc_ca_solver_info info;
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1) 
	{
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help forces_pro_mpc_ca_solver_mex' for details.");
    }    
	if (nlhs > 3) 
	{
	...
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [140 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,140);
    ...
    if( mxGetM(par) != 380 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [380 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.all_parameters,380);

	}


	#if SET_PRINTLEVEL_forces_pro_mpc_ca_solver > 0
	...
	/* call solver */
	exitflag = forces_pro_mpc_ca_solver_solve(&params, &output, &info, fp, pt2function_forces_pro_mpc_ca_solver);
