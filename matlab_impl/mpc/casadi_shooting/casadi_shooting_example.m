...
problem_setup;
recompile = false;
solver_build_name = 'casadi_shooting_nlp';
...
folder_name = './matlab_impl/mpc/casadi_shooting/build/';
if recompile
...
mpc_u_plan = zeros(model.nu, model.N);
...
while n_loop <= max_n_loop && flag_robot_reach == 0
    ...
    if mpc_infeasible == 0      % feasible
        nlp_x0 = [robot_pos_current; robot_vel_current];
        for i = 1 : model.N-1
            u_x_s_temp = [mpc_u_plan(:, i+1); mpc_x_plan(:, i+2); zeros(model.ns, 1)];
            nlp_x0 = [nlp_x0; u_x_s_temp];
        end
        nlp_x0 = [nlp_x0; u_x_s_temp];
    else                        % infeasible
        u_x_s_temp = [zeros(model.nu,1); robot_pos_current; robot_vel_current; zeros(model.ns, 1)];
        nlp_x0 = [robot_pos_current; robot_vel_current; repmat(u_x_s_temp, model.N, 1)];
    end
    % call the solver
    tic;
...
%             'lbg', nlp_lbg, 'ubg', nlp_ubg);  % not using code generation
    ...
    for i = 1 : model.N
        u_x_s_temp = x_opt(model.nx+(model.nu+model.nx+model.ns)*(i-1)+1 : ...
            model.nx+(model.nu+model.nx+model.ns)*(i));
        mpc_u_plan(:, i) = u_x_s_temp(1:model.nu);
        mpc_x_plan(:, i+1) = u_x_s_temp(model.nu+1 : model.nu+model.nx);
        mpc_s_plan(:, i) = u_x_s_temp(model.nu+model.nx+1 : model.nu+model.nx+model.ns);
    end
